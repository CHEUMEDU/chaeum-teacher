...
//   · 5단계 채점 기준 (A~E)
//   · 배치 채점 모드 (1학생 = 1회 호출 → 비용 1/5)
//   · 빠른 일치 체크 (대소문자/공백/문장부호 차이만 → 즉시 100점)
//   · 빈칸 N개 (파이프 |) 분리 채점 + 평균 점수
//   · 429 (분당 한도) 5초 후 1회 재시도
//
// 호출 방법:
//   POST { items: [{q, studentAnswer, correctAnswer, questionContext}, ...] }
...
export default async function handler(req) {
  ...
  if (!correctAnswer) {
    return jsonResponse({ ok: false, error: '정답 데이터 없음', version: VERSION }, 400);
  }
...
async function handleBatchGrade(items) {
  ...
  return jsonResponse({ ok: true, results, version: VERSION });
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return items.map(it => ({
        ...
        score: 0, category: "ERROR",
        deductions: [],
        reasoning: `Gemini HTTP ${res.status} — 분당 호출 한도 초과 가능. 잠시 후 재시도하세요.`
      }));
    }
